...
export default async function SuccessPage({ searchParams }: { searchParams: { [key: string]: string } }) {
  ...
  return (
    <div id="payment-status" className='flex flex-col items-center justify-end gap-2 p-6 '>
      <div className='flex flex-row items-center justify-center gap-4 mb-4'>
      <div className='' id="status-icon" style={{backgroundColor: statusContent.iconColor}}>
        {statusContent.icon}
      </div>
      <h2 className='text-2xl font-bold' id="status-text">{statusContent.text}</h2>
      </div>
      ...
      {paymentIntent && <a href={`https://dashboard.stripe.com/payments/${paymentIntentId}`} id="view-details" target="_blank">
        <svg className='text-lg' width="15" height="14" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg" style={{paddingLeft: '5px'}}>
